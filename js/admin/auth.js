...
function showApp(){document.getElementById('login-wrap').style.display='none';document.getElementById('app').classList.add('on');isSuperAdmin=admin.role==='super_admin';document.getElementById('role-badge').textContent=admin.role==='super_admin'?'Super Admin':admin.role==='admin'?'Admin':'Operations';document.getElementById('btn-new-acct').style.display=isSuperAdmin?'block':'none';try{initMap()}catch(e){console.error('initMap:',e)};loadData();loadSettings();setInterval(loadData,3000);setInterval(heartbeat,30000);heartbeat()}
