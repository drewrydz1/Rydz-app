...
var topD=drvs.map(function(d){var c=rides.filter(function(r){return r.driver_id===d.id&&r.status==='completed'}).length;return{n:d.name,c:c}}).sort(function(a,b){return b.c-a.c}).slice(0,5);
document.getElementById('dash-drv').innerHTML=topD.map(function(d){return dss(esc(d.n||'--'),d.c)}).join('');
