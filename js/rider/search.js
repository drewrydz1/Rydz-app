...
function buildResultItem(p, k) {
  ...
  return '<div class="ac-item" onclick="selPlace(this)" data-pid="' + p.place_id + '" data-k="' + k + '">' +
...
window.chkBtn = function() {
  ...
  var ok = puSel && doSel && db && db.settings.serviceStatus;
  b.disabled = !ok;
  b.textContent = ok ? 'Continue' : (!puSel && !doSel ? 'Select pickup & drop-off' : !puSel ? 'Select a pickup location' : 'Select a drop-off');
}
