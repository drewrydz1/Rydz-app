...
// Uses login timestamp in localStorage so sessions survive app backgrounding.
// Only explicit logout, disabled account, or crossing 5am ET signs out.
function _last5amET(){
  var now=new Date();
...
function _scheduleNext5amLogout(){
  ...
  if(nh===4)next5+=60*60*1000;
  if(nh===6)next5-=60*60*1000;
  var delay=next5-now;
...
// Auto sign-out at 5am America/New_York: expire sessions that began before the
// most recent 5am boundary.
...
var _sid=localStorage.getItem('rydz-drv-id');if(_sid){DID=_sid;if(db){var _sd=gD();if(_sd&&!_sd.disabled){document.getElementById('h-nm').textContent=_sd.name;document.getElementById('h-av').textContent=(_sd.name||'D')[0];document.getElementById('h-vc').textContent=_sd.vehicle||'';setInterval(poll,5000);setTimeout(supaSync,2000);setInterval(supaSync,3000);if(localStorage.getItem('rydz-drv-online')==='true'){startGPS()}_scheduleNext5amLogout();if(typeof subscribeDriverRealtime==='function')subscribeDriverRealtime();go('main');ren();return}}else{setInterval(poll,5000);setTimeout(supaSync,2000);setInterval(supaSync,3000);if(localStorage.getItem('rydz-drv-online')==='true'){startGPS()}_scheduleNext5amLogout();if(typeof subscribeDriverRealtime==='function')subscribeDriverRealtime();go('main');supaSync();setTimeout(ren,2000);return}}go('login');
setInterval(poll,5000);
setTimeout(supaSync,2000);
setInterval(supaSync,3000);if(localStorage.getItem('rydz-drv-online')==='true'){startGPS()}
}
